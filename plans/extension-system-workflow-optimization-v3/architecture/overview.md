...
│ • ha-common-*    │  │                  │
└──────────────────┘  └──────────────────┘
  Auto-invoked via       Invoked by commands
  hooks (UserPrompt      (Task tool)
  + file context)
```
...
- **Auto-activated via hook system** (not Claude's built-in auto-triggering)
- Provide context without explicit invocation
- Progressive disclosure (main Skill loads first, details on-demand)

**When They Activate:**
- User prompt contains trigger keywords (e.g., "entity", "sensor")
- User edits relevant files (e.g., `sensor.py`, `manifest.json`)
- Command explicitly activates them

**Example Flow:**
1. User types: "How do I create a sensor with unique ID?"
2. Hook detects "sensor" and "unique ID" keywords
3. Hook auto-activates `ha-entity-knowledge` Skill
...
**See:** [Hook-Based Activation System](hook-system.md) for how Skills auto-activate
...
- Skills load automatically when needed (hook-based)
...
**Key Difference:** Skills' hook-based activation is **reliable and automatic**, while thoughts/ requires manual reference and loading.

**Critical Note:** Skills' built-in auto-triggering (based on SKILL.md descriptions) doesn't work reliably. This architecture uses a **hook-based activation system** for reliable, context-aware Skill activation.

**See:** [Hook-Based Activation System](hook-system.md)
...
skill-rules.json                         # Hook trigger patterns (root or .claude/)
```
...
**Solution:** Progressive disclosure through hook-based activation
...
| v2 (Skills - built-in auto) | Skills auto-load (unreliable) | ⚠️ Medium-High |
| **v3 (Skills + Hooks)** | **Skills auto-load reliably** | ✅ **High** |

### How Hooks Optimize Context

1. **Prompt-based activation:** Analyze user prompt for keywords
2. **File-based activation:** Check which files user is editing
3. **Selective loading:** Only load Skills relevant to current task
4. **Progressive detail:** Load main Skill first, supporting docs on-demand

**Example:**
- User edits `sensor.py`: Hook loads `ha-entity-knowledge` only
- User edits `config_flow.py`: Hook loads `ha-config-flow-knowledge` only
- User asks "How do coordinators work?": Hook loads `ha-coordinator-knowledge` only
...
1. **Understand hooks:** Read [Hook-Based Activation System](hook-system.md)
2. **See interaction details:** Read [Component Interaction Model](component-interaction.md)
3. **Start building:** Read [Phase 1: Skills Foundation](../phases/phase-1-skills.md)
